import { Translations } from '@/i18n/types'

const ptTranslations: { translations: Translations } = {
  translations: {
    dashboard: 'Painel',
    events: 'Eventos',
    bookings: 'Reservas',
    flights: 'Voos',
    profile: 'Perfil',
    users: 'Usuários',
    logout: 'Sair',
    pilotLogin: 'Login Piloto',
    adminLogin: 'Login Admin',
    jalPilotId: 'ID Piloto JAL',
    email: 'Email',
    password: 'Senha',
    login: 'Entrar',
    signingIn: 'Entrando...',
    switchToAdmin: 'Mudar para Admin',
    switchToPilot: 'Mudar para Piloto',
    upcomingEvents: 'Próximos Eventos',
    pastEvents: 'Eventos Passados',
    createEvent: 'Criar Evento',
    editEvent: 'Editar Evento',
    deleteEvent: 'Excluir Evento',
    eventsFound: 'Encontrados {count} eventos',
    loadMoreEvents: 'Carregar Mais Eventos',
    noEventsFound: 'Nenhum evento encontrado',
    eventType: 'Tipo de Evento',
    eventStatus: 'Status do Evento',
    created: 'Criado',
    active: 'Ativo',
    completed: 'Concluído',
    cancelled: 'Cancelado',
    eventTitle: 'Título do Evento',
    eventDescription: 'Descrição do Evento',
    eventDate: 'Data do Evento',
    startTime: 'Hora de Início',
    endTime: 'Hora de Fim',
    departure: 'Partida',
    arrival: 'Chegada',
    aircraft: 'Aeronave',
    maxSlots: 'Slots Máximos',
    availableSlots: 'Slots Disponíveis',
    bookedSlots: 'Slots Reservados',
    eventDetails: 'Detalhes do Evento',
    eventSlots: 'Slots do Evento',
    pilotBriefing: 'Briefing do Piloto',
    atcBriefing: 'Briefing da ATC',
    sceneries: 'Cenários',
    freeware: 'Gratuito',
    payware: 'Pago',
    bookSlot: 'Reservar Slot',
    slotType: 'Tipo de Slot',
    landing: 'Pouso',
    private: 'Privado',
    flightNumber: 'Número do Voo',
    airline: 'Companhia Aérea',
    aircraftType: 'Tipo de Aeronave',
    gate: 'Portão',
    terminal: 'Terminal',
    status: 'Status',
    onTime: 'No Horário',
    delayed: 'Atrasado',
    noFlightsFound: 'Nenhum Voo Encontrado',
    unableToBook: 'Não é Possível Reservar',
    filter: 'Filtrar',
    clear: 'Limpar',
    search: 'Pesquisar',
    myBookings: 'Minhas Reservas',
    allBookings: 'Todas as Reservas',
    createBooking: 'Criar Reserva',
    cancelBooking: 'Cancelar Reserva',
    bookingStatus: 'Status da Reserva',
    pending: 'Pendente',
    confirmed: 'Confirmado',
    rejected: 'Rejeitado',
    departures: 'Partidas',
    arrivals: 'Chegadas',
    personalInfo: 'Informações Pessoais',
    firstName: 'Nome',
    lastName: 'Sobrenome',
    callsign: 'Callsign',
    jalId: 'ID JAL',
    role: 'Função',
    pilot: 'Piloto',
    admin: 'Admin',
    allUsers: 'Todos os Usuários',
    searchUsers: 'Buscar Usuários',
    userRole: 'Função do Usuário',
    joinDate: 'Data de Entrada',
    lastActive: 'Último Ativo',
    save: 'Salvar',
    cancel: 'Cancelar',
    delete: 'Excluir',
    edit: 'Editar',
    create: 'Criar',
    loading: 'Carregando...',
    error: 'Erro',
    success: 'Sucesso',
    warning: 'Aviso',
    info: 'Informação',
    yes: 'Sim',
    no: 'Não',
    confirm: 'Confirmar',
    loginSuccess: 'Login realizado com sucesso',
    loginError: 'Falha no login',
    logoutSuccess: 'Logout realizado com sucesso',
    logoutError: 'Falha no logout',
    bookingCreated: 'Reserva criada com sucesso',
    bookingCancelled: 'Reserva cancelada',
    eventCreated: 'Evento criado com sucesso',
    eventUpdated: 'Evento atualizado com sucesso',
    eventDeleted: 'Evento excluído com sucesso',
    profileUpdated: 'Perfil atualizado com sucesso',
    
    // Errors
    errors: {
      general: {
        title: 'Algo deu errado',
        subtitle: 'Ocorreu um erro inesperado. Tente novamente.'
      },
      network: {
        title: 'Erro de conexão',
        subtitle: 'Não foi possível conectar ao servidor. Verifique sua conexão com a internet.'
      },
      auth: {
        title: 'Erro de autenticação',
        subtitle: 'Sua sessão expirou. Redirecionando para o login...'
      },
      notFound: {
        title: 'Página não encontrada',
        subtitle: 'A página que você está procurando não existe ou foi movida.'
      },
      retry: 'Tentar novamente',
      backToHome: 'Voltar ao início',
      backToDashboard: 'Voltar ao painel'
    },
    
    // Cookie Consent
    cookies: {
      title: 'Consentimento de cookies',
      subtitle: 'Usamos cookies para melhorar sua experiência e analisar nosso tráfego. Ao continuar usando nosso site, você consente com nosso uso de cookies.',
      authorizeUse: 'Aceitar todos os cookies',
      continueWithout: 'Continuar sem cookies',
      learnMore: 'Saiba mais',
      necessary: 'Necessário',
      analytics: 'Analíticas',
      marketing: 'Marketing',
      preferences: 'Preferências'
    },
    
    // Slot Notifications
    slotConfirmed: 'Slot Confirmado',
    slotConfirmedSubtitle: 'Seu slot foi confirmado com sucesso!',
    slotScheduled: 'Slot Agendado',
    slotScheduledSubtitle: 'Seu slot foi agendado com sucesso!',
    slotCancelled: 'Slot Cancelado',
    slotCancelledSubtitle: 'Seu slot foi cancelado.',
    scheduleConfirmation: 'Confirmação de Agendamento',
    scheduleConfirmationSubtitle: 'Por favor confirme sua reserva de slot',
    scheduleConfirmationAlert: 'Por favor certifique-se de que todas as informações estão corretas antes de confirmar.',
    confirmSchedule: 'Confirmar Agendamento',
    
    // Slot Actions
    confirmFlight: 'Confirmar Voo',
    cancelFlight: 'Cancelar Voo',
    waitToConfirm: 'Aguardar Confirmação',
    cancelFlightConfirmation: 'Tem certeza de que deseja cancelar este voo?',
    backToSlots: 'Voltar aos Meus Slots',
    
    copyright: '© 2024 Japan Airlines Virtual',
    jalVirtual: 'JAL Virtual',
    bookingSystem: 'Sistema de Reservas',
  },
}

export default ptTranslations
